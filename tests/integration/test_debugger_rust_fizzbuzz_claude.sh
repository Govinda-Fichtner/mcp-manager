#!/bin/bash
# Integration test: debugger-rust with Claude Code using FizzBuzz
# Tests the Rust-specific debugger MCP server
...
TEST_WORKSPACE="$SCRIPT_DIR/workspace_rust"
MCP_SERVER_NAME="debugger-rust-test"
...
# Main test
echo "================================================"
echo "FizzBuzz Integration Test: debugger-rust + Claude Code"
echo "================================================"
echo ""
...
# Step 3: Check debugger-rust image
log_step "Step 3: Validating debugger-rust Docker image"
if ! docker images | grep -q "debugger-mcp-rust"; then
    log_error "debugger-mcp-rust image not found"
    log_info "Run: ./mcp_manager.sh setup debugger-rust"
    exit 1
fi
log_success "debugger-rust image found"
...
# Create Cargo.toml
cat > "$TEST_WORKSPACE/Cargo.toml" << 'CARGO_TOML'
[package]
name = "fizzbuzz"
version = "0.1.0"
edition = "2021"
...
# Create Rust FizzBuzz with bug
cat > "$TEST_WORKSPACE/src/main.rs" << 'RUST_CODE'
...
fn fizzbuzz(n: i32) -> String {
    ...
    if n % 4 == 0 {  // <-- INTENTIONAL BUG HERE (line 18)
...
log_success "Created FizzBuzz test files: $TEST_WORKSPACE/src/main.rs"
log_info "Bug location: Line 18 in src/main.rs (uses % 4 instead of % 5)"

# Step 5: Generate MCP configuration
log_step "Step 5: Generating MCP server configuration"
cd "$PROJECT_ROOT"

mcp_config_json="$(./mcp_manager.sh config debugger-rust --add-json --env-file .env)"
...
log_step "Step 6: Adding debugger-rust server to Claude Code"
...
log_info "Claude should use debugger_mcp tools to inspect the Rust code"

# Create prompt file
cat > "$TEST_WORKSPACE/debug_prompt.txt" << 'PROMPT'
Analyze the Rust FizzBuzz implementation in src/main.rs and find the bug.
...
Use the debugger-rust-test MCP tools if available to:
1. Start a debugging session for the Rust binary
2. Set a breakpoint at line 18 in src/main.rs (the bug location)
...
2. What the bug is
3. Where the bug is located (file and line number)
4. How to fix it
PROMPT

log_info "Prompt: Analyze the FizzBuzz implementation in src/main.rs and find the bug."
...
# Check if Claude found the location (line 18 or % 4)
if grep -Ei "line.*18|18.*line|% 4|modulo 4|mod 4|main\.rs" "$claude_output_file"; then
    log_success "Claude identified the bug location (line 18 or % 4)"
else
...
if grep -Ei "line.*18|% 4" "$claude_output_file"; then
    echo -e "${GREEN}✓${NC}"
else
    echo -e "${RED}✗${NC}"
fi
